...
d1 = do
    ...
    let (Right s1) = s
    let s2 = tryAddColumn s1 1 ("Origin", CString (CData ["Africa", "India", "Brazil", "India", "China"]))
...
aggrDemo :: IO ()
aggrDemo =  do
    Right sold <- importCSV "demo/products.csv" ',' True
    Right prod <- importCSV "demo/product_list.csv" ',' True
